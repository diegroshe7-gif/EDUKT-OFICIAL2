...
export default function TeacherCalendar({ tutorId }: TeacherCalendarProps) {
        ...
        <div>
          ...
          <div className="grid gap-4">
            {pastSesiones.map((sesion) => (
              <SessionCard key={sesion.id} sesion={sesion} isPast />
            ))}
          </div>
        </div>
      )}
...
function SessionCard({ sesion, isPast = false }: SessionCardProps) {
  const { data: alumno } = useQuery<Alumno>({
    queryKey: ["/api/alumnos", sesion.alumnoId],
    enabled: !!sesion.alumnoId,
  });
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <CardTitle className="text-lg">
                Sesión con {alumno ? `${alumno.nombre} ${alumno.apellido}` : "Estudiante"}
              </CardTitle>
              {isToday && !isPast && (
            ...
            <CardDescription className="space-y-1">
              ...
              {alumno && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span data-testid={`text-student-email-${sesion.id}`}>{alumno.email}</span>
                </div>
              )}
            </CardDescription>
          </div>
