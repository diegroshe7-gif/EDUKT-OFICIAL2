...
- **Password Reset:** Token-based password reset system implemented with 1-hour expiration, stored in `reset_tokens` database table. Email delivery via Gmail API using the same Google Calendar connection, sending from notificationsedukt@gmail.com to any user's email with the reset code.
