...
- **File Upload System:** Integrated Replit App Storage with Google Cloud Storage backend for handling file uploads. Tutor registration form features full drag & drop uploading for profile photos (up to 5MB, image formats) and CV documents (up to 10MB, PDF/Word formats). Uses Uppy Core with AwsS3 plugin for client-side uploads to presigned URLs (public uploading pattern). Features include drag-and-drop interface, click-to-upload fallback, file type/size validation, progress indicators, and visual upload status feedback. Files are stored in object storage and served via `/objects/` endpoint.
...
- **File Storage:** Replit App Storage (Google Cloud Storage backend) for profile photos and CV documents. Uses `@google-cloud/storage` for server-side operations and `@uppy/core@5`, `@uppy/react@5`, `@uppy/aws-s3@5` for client-side drag & drop uploads. ObjectUploader component implements drag-and-drop using Uppy Core with useUppyState hook for progress tracking and visual feedback. Environment variables: `PUBLIC_OBJECT_SEARCH_PATHS`, `PRIVATE_OBJECT_DIR`, `DEFAULT_OBJECT_STORAGE_BUCKET_ID`.
